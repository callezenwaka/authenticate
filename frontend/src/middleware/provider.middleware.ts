...
export const providerMiddleware = async (
  ...
  try {
    ...
    if (request.tokens) {
      await serviceProvider.initialize(request.tokens);
    ...
    request.services = serviceProvider;

    // Listen for response finish to update session if tokens were refreshed
    res.on('finish', () => {
      if (request.tokens !== serviceProvider.getTokens() && serviceProvider.getTokens()) {
        // Update tokens in session if they changed
        req.session.tokens = serviceProvider.getTokens();
